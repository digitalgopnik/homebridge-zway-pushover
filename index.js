...
const debug = require("debug")("ZWayServer");
...
function ZWayServerPlatform(log, config) {
  ...
  this.optIn = config["opt_in"];
  this.pushoverCredentials = config["pushover"] || {};
  this.nameOverrides = config["name_overrides"];
  ...
  this.splitServices =
    config["split_services"] === undefined ? true : config["split_services"];
  this.dimmerOffThreshold =
    config["dimmer_off_threshold"] === undefined
      ? 5
      : config["dimmer_off_threshold"];
  this.lastUpdate = 0;
  this.cxVDevMap = {};
  this.vDevStore = {};
  this.sessionId = "";
  this.jar = request.jar(new tough.CookieJar());
  if (Object.keys(this.pushoverCredentials).length !== 0) {
    this.pushoverClient = new Pushover(
      this.pushoverCredentials["user"],
      this.pushoverCredentials["token"]
    );
  }
}
...
module.exports = function (homebridge) {
  ...
  ZWayServerPlatform.CurrentPowerConsumption.UUID =
    "E863F10D-079E-48FF-8F27-9C2605A29F52";
  ...
  ZWayServerPlatform.TotalPowerConsumption.UUID =
    "E863F10C-079E-48FF-8F27-9C2605A29F52";
  ...
  for (var serviceKey in Service)
    if (Service[serviceKey].UUID != undefined)
      ZWayServerPlatform.ServiceUUIDReverseLookupMap[Service[serviceKey].UUID] =
        serviceKey;
  for (var serviceKey in ZWayServerPlatform)
    if (ZWayServerPlatform[serviceKey].UUID != undefined)
      ZWayServerPlatform.ServiceUUIDReverseLookupMap[
        ZWayServerPlatform[serviceKey].UUID
      ] = serviceKey;
  ...
  ZWayServerAccessory.prototype.extraCharacteristicsMap = {
    ...
    "sensorMultilevel.meterElectric_watt": [
      ZWayServerPlatform.CurrentPowerConsumption,
    ],
    "sensorMultilevel.meterElectric_kilowatt_per_hour": [
      ZWayServerPlatform.TotalPowerConsumption,
    ],
  };

  homebridge.registerAccessory(
    "homebridge-zway-pushover",
    "ZWayServer",
    ZWayServerAccessory
  );
...
ZWayServerPlatform.getVDevTypeKeyRoot = function (vdev) {
  ...
  if (
    (overrideDeviceType = ZWayServerPlatform.prototype.getTagValue(
      vdev,
      "Override.deviceType"
    ))
  ) {
...
ZWayServerPlatform.getVDevTypeKey = function (vdev) {
  ...
  if (
    (overrideProbeType = ZWayServerPlatform.prototype.getTagValue(
      vdev,
      "Override.probeType"
    ))
  ) {
...
ZWayServerPlatform.prototype = {
    ...
    request(opts, function (error, response, body) {
      if (response && response.statusCode == 401) {
        ...
        const resp = response
          ...
          : "";
        const err = error ? `Error code ${error.code}.` : "";
        that.log(
  ...
  pushover: function (message, title = "Error") {
    if (
      Object.keys(this.pushoverCredentials).length !== 0 &&
      this.pushoverClient
    ) {
      this.pushoverClient
  ...
  isVDevBridged: function (vdev) {
    ...
    if (this.optIn) isBridged = false;
    else isBridged = true; // Start with the initial bias
    ...
    if (this.optIn && isTaggedInclude) isBridged = true; // ...unless we're in optIn mode, where Include always wins.

    if (!isBridged && !this.optIn) {
      debug("Device " + vdev.id + " skipped! ");
      debug({
        permanently_hidden: vdev.permanently_hidden,
        Skip: isTaggedSkip,
        Include: isTaggedInclude,
        opt_in: this.optIn,
      });
    }
    return isBridged;
    ...
    for (var gdid in groupedDevices) {
      ...
      for (var j = 0; j < gd.devices.length; j++) {
        debug(
          gd.devices[j].id +
            " - " +
            gd.devices[j].deviceType +
            (gd.devices[j].metrics && gd.devices[j].metrics.probeTitle
              ? "." + gd.devices[j].metrics.probeTitle
              : "")
        );
      }
      ...
      if (gd.primary !== undefined) {
        ...
        for (var ti = 0; ti < primaryDeviceClasses.length; ti++) {
          if (gd.types[primaryDeviceClasses[ti]] !== undefined) {
            ...
            var pd = gd.devices[gd.primary];
            var name =
              pd.metrics && pd.metrics.title ? pd.metrics.title : pd.id;
        ...
        if (this.cxVDevMap[upd.id]) {
          ...
          for (var j = 0; j < cxs.length; j++) {
            ...
            if (oldValue !== newValue) {
              ...
              cx.emit("update", {
                oldTimestamp: vdev.updateTime,
                newTimestamp: upd.updateTime,
              });
...
ZWayServerAccessory.prototype = {
    ...
    switch (typeKey) {
      ...
      case "switchBinary":
        if (this.platform.getTagValue(vdev, "Service.Type") === "Lightbulb") {
          ...
          this.platform.getTagValue(vdev, "Service.Type") === "Outlet"
        ) {
          services.push(new Service.Outlet(vdev.metrics.title, vdev.id));
        } else if (
          this.platform.getTagValue(vdev, "Service.Type") === "WindowCovering"
        ) {
          services.push(
            new Service.WindowCovering(vdev.metrics.title, vdev.id)
          );
        } else {
      ...
      case "sensorBinary.alarm_door":
        ...
        if (stype === "ContactSensor") {
          ...
          services.push(
            new Service.GarageDoorOpener(vdev.metrics.title, vdev.id)
          );
      ...
      case "sensorMultilevel.Temperature":
        services.push(
          new Service.TemperatureSensor(vdev.metrics.title, vdev.id)
        );
        break;
      ...
      case "toggleButton":
        ...
        if (stype === "StatefulProgrammableSwitch") {
          services.push(
            new Service.StatefulProgrammableSwitch(vdev.metrics.title, vdev.id)
          );
        } else {
          services.push(
            new Service.StatelessProgrammableSwitch(vdev.metrics.title, vdev.id)
          );
        }
        break;
    }
  ...
  getVDevForCharacteristic: function (cx, vdevPreferred) {
    ...
    if (!map) {
      ...
      map[new Characteristic.OutletInUse().UUID] = [
        "sensorMultilevel.meterElectric_watt",
        "switchBinary",
      ];
      ...
      map[new Characteristic.CurrentTemperature().UUID] = [
        "sensorMultilevel.Temperature",
        "thermostat",
      ];
      map[new Characteristic.CurrentRelativeHumidity().UUID] = [
        "sensorMultilevel.Humidity",
      ];
      map[new Characteristic.TargetTemperature().UUID] = ["thermostat"];
      map[new Characteristic.TemperatureDisplayUnits().UUID] = [
        "sensorMultilevel.Temperature",
        "thermostat",
      ]; //TODO: Always a fixed result
      map[new Characteristic.CurrentHeatingCoolingState().UUID] = [
        "thermostat",
      ]; //TODO: Always a fixed result
      map[new Characteristic.TargetHeatingCoolingState().UUID] = ["thermostat"]; //TODO: Always a fixed result
      map[new Characteristic.CurrentDoorState().UUID] = [
        "sensorBinary.Door/Window",
        "sensorBinary",
      ];
      map[new Characteristic.TargetDoorState().UUID] = [
        "sensorBinary.Door/Window",
        "sensorBinary",
      ]; //TODO: Always a fixed result
      map[new Characteristic.ContactSensorState().UUID] = [
        "sensorBinary",
        "sensorBinary.Door/Window",
      ]; //NOTE: A root before a full...what we want?
      map[new Characteristic.LeakDetected().UUID] = [
        "sensorBinary.alarmSensor_flood",
        "sensorBinary.General purpose",
        "sensorBinary",
      ];
      map[new Characteristic.CurrentPosition().UUID] = [
        "sensorBinary.Door/Window",
        "switchMultilevel.blind",
        "switchBinary.motor",
        "sensorBinary",
        "switchMultilevel",
        "switchBinary",
      ]; // NOTE: switchBinary.motor may not exist...guessing?
      map[new Characteristic.TargetPosition().UUID] = [
        "sensorBinary.Door/Window",
        "switchMultilevel.blind",
        "switchBinary.motor",
        "sensorBinary",
      ...
      map[new Characteristic.PositionState().UUID] = [
        "sensorBinary.Door/Window",
        "switchMultilevel.blind",
        "switchBinary.motor",
        "sensorBinary",
      ...
      map[new Characteristic.HoldPosition().UUID] = [
        "switchMultilevel.blind",
        "switchBinary.motor",
        "switchMultilevel",
      ]; // NOTE: switchBinary.motor may not exist...guessing?
      map[new Characteristic.ObstructionDetected().UUID] = [
        "sensorBinary.Door/Window",
        "sensorBinary",
      ]; //TODO: Always a fixed result
      map[new Characteristic.SmokeDetected().UUID] = [
        "sensorBinary.alarm_smoke",
        "sensorBinary.alarm_heat",
      ];
      ...
      map[new Characteristic.CurrentAmbientLightLevel().UUID] = [
        "sensorMultilevel.Luminiscence",
      ];
      ...
      map[new Characteristic.ProgrammableSwitchOutputState().UUID] = [
        "toggleButton",
      ];
      map[new ZWayServerPlatform.CurrentPowerConsumption().UUID] = [
        "sensorMultilevel.meterElectric_watt",
      ];
      map[new ZWayServerPlatform.TotalPowerConsumption().UUID] = [
        "sensorMultilevel.meterElectric_kilowatt_per_hour",
      ];
    }
    ...
    if (
      vdevPreferred &&
      typekeys.indexOf(ZWayServerPlatform.getVDevTypeKey(vdevPreferred)) >= 0
    ) {
    ...
    for (var i = 0; i < typekeys.length; i++) {
      for (var j = 0; j < candidates.length; j++) {
        if (ZWayServerPlatform.getVDevTypeKey(candidates[j]) === typekeys[i])
          return candidates[j];
        // Also try the "root" key, e.g. sensorBinary vs. sensorBinary.general_purpose ...
        if (
          ZWayServerPlatform.getVDevTypeKeyRoot(candidates[j]) === typekeys[i]
        )
          return candidates[j];
      }
    }
  ...
  configureCharacteristic: function (cx, vdev, service) {
    ...
    if (!this.platform.cxVDevMap[vdev.id])
      this.platform.cxVDevMap[vdev.id] = [];
    this.platform.cxVDevMap[vdev.id].push(cx);
    if (!this.platform.vDevStore[vdev.id])
      this.platform.vDevStore[vdev.id] = vdev;
    ...
    var descOverride = this.platform.getTagValue(
      vdev,
    ...
    if (cx instanceof Characteristic.On) {
      cx.zway_getValueFromVDev = function (vdev) {
        ...
        if (vdev.metrics.level === "on") {
          ...
          vdev.metrics.level <= accessory.platform.dimmerOffThreshold
        ) {
          val = false;
      ...
      cx.on(
        ...
        function (callback, context) {
          ...
          this.getVDev(vdev).then(function (result) {
            debug(
              "Got value: " +
                cx.zway_getValueFromVDev(result.data) +
                ", for " +
                vdev.metrics.title +
                "."
            );
    ...
    if (cx instanceof Characteristic.OutletInUse) {
      cx.zway_getValueFromVDev = function (vdev) {
        ...
        if (vdev.metrics.level === "on") {
          ...
          var t =
            this.platform.getTagValue(vdev, "OutletInUse.Level") ||
            this.platform.OIUWatts;
    ...
    if (
      cx instanceof Characteristic.Brightness ||
      cx instanceof Characteristic.RotationSpeed
    ) {
      cx.zway_getValueFromVDev = function (vdev) {
      ...
      cx.on(
        ...
        interlock(
          function (level, callback) {
            this.command(vdev, "exact", { level: parseInt(level, 10) }).then(
              function (result) {
                callback();
              }
            );
    ...
    if (cx instanceof Characteristic.Hue) {
      cx.zway_getValueFromVDev = function (vdev) {
        debug(
          "Derived value " +
            accessory.rgb2hsv(vdev.metrics.color).h +
            " for hue."
        );
    ...
    if (cx instanceof Characteristic.Saturation) {
      cx.zway_getValueFromVDev = function (vdev) {
        debug(
          "Derived value " +
            accessory.rgb2hsv(vdev.metrics.color).s +
            " for saturation."
        );
      ...
      cx.on(
        "set",
        interlock(
          function (saturation, callback) {
            var hcx = service.getCharacteristic(Characteristic.Hue);
            var vcx = service.getCharacteristic(Characteristic.Brightness);
            if (!hcx || !vcx) {
              debug(
                "Saturation without Hue and Brightness is not supported! Cannot set value!"
              );
              callback(true, cx.value);
            }
            var rgb = this.hsv2rgb({
              h: hcx.value,
              s: saturation,
              v: vcx.value,
            });
    ...
    if (cx instanceof Characteristic.SmokeDetected) {
      ...
      cx.on("update", function (ev) {
        ...
        setTimeout(function () {
          cx.emit("change", { oldValue: inverse, newValue: value });
    ...
    if (cx instanceof Characteristic.CurrentTemperature) {
      ...
      cx.setProps({
        minValue:
          vdev.metrics && vdev.metrics.min !== undefined
            ? vdev.metrics.min
            : -40,
        maxValue:
          vdev.metrics && vdev.metrics.max !== undefined
            ? vdev.metrics.max
            : 999,
      });
      return cx;
    ...
    if (cx instanceof Characteristic.TargetTemperature) {
      ...
      cx.setProps({
        minValue:
          vdev.metrics && vdev.metrics.min !== undefined ? vdev.metrics.min : 5,
        maxValue:
          vdev.metrics && vdev.metrics.max !== undefined
            ? vdev.metrics.max
            : 40,
      ...
      cx.on(
        ...
        interlock(
          function (newValue, callback) {
            debug(
              "WARN: Set of TargetHeatingCoolingState not yet implemented, resetting to HEAT!"
            );
    ...
    if (cx instanceof Characteristic.StatusLowBattery) {
      cx.zway_getValueFromVDev = function (vdev) {
        return vdev.metrics.level <= accessory.platform.batteryLow
    ...
    if (cx instanceof Characteristic.StatusTampered) {
      ...
      cx.on("update", function (ev) {
        var value = cx.value;
        var inverse =
          value == Characteristic.StatusTampered.NOT_TAMPERED
            ? Characteristic.StatusTampered.TAMPERED
            : Characteristic.StatusTampered.NOT_TAMPERED;
        cx.emit("change", { oldValue: value, newValue: inverse });
        setTimeout(function () {
    ...
    if (cx instanceof Characteristic.ContactSensorState) {
      cx.zway_getValueFromVDev = function (vdev) {
        ...
        boolval = accessory.platform.getTagValue(
          vdev,
          "ContactSensorState.Invert"
        )
          ? !boolval
          : boolval;
        return boolval
    ...
    if (cx instanceof Characteristic.LeakDetected) {
      ...
      cx.on("update", function (ev) {
        var value = cx.value;
        var inverse =
          value == Characteristic.LeakDetected.LEAK_NOT_DETECTED
            ? Characteristic.LeakDetected.LEAK_DETECTED
            : Characteristic.LeakDetected.LEAK_NOT_DETECTED;
        cx.emit("change", { oldValue: value, newValue: inverse });
        setTimeout(function () {
    ...
    if (cx instanceof Characteristic.TargetPosition) {
      cx.zway_getValueFromVDev = function (vdev) {
        if (service instanceof Service.WindowCovering) {
          ...
          if (this.value !== cx.getDefaultValue())
            return this.value == 99 ? 100 : this.value;
        ...
        interlock(
          function (level, callback) {
            if (isNaN(vdev.metrics.level)) {
              ...
              this.command(vdev, level == 0 ? "off" : "on")
                .then(function (result) {
                  //debug("Got value: " + result.data.metrics.level + ", for " + vdev.metrics.title + ".");
                  callback(false);
                })
              ...
              switch (parseInt(level, 10)) {
                ...
                case 100:
                  ...
                  break;
                default:
                  promise = this.command(vdev, "exact", {
      ...
      cx.setProps({
        minValue:
          vdev.metrics && vdev.metrics.min !== undefined ? vdev.metrics.min : 0,
        maxValue:
          vdev.metrics &&
          vdev.metrics.max !== undefined &&
          vdev.metrics.max != 99
            ? vdev.metrics.max
            : 100,
      });
    ...
    if (cx instanceof Characteristic.HoldPosition) {
      cx.on("get", function (callback, context) {
        debug(
          "WARN: Getting value for read-only HoldPosition Characteristic on " +
            vdev.metrics.title +
            "...should this happen?"
        );
        callback(false, null);
    ...
    if (cx instanceof Characteristic.LockTargetState) {
      cx.zway_getValueFromVDev = function (vdev) {
        ...
        debug(
          'Returning LockTargetState of "' +
            val +
            '" because vdev.metrics.level returned "' +
            vdev.metrics.level +
            '"'
        );
        return val;
    ...
    if (cx instanceof Characteristic.ProgrammableSwitchEvent) {
      ...
      cx.on("update", function (ev) {
        cx.emit("change", {
          oldValue: cx.value,
          newValue: (cx.value = cx.zway_setValueOnVDev(cx.value + 1)),
        });
        if (service.UUID === Service.StatelessProgrammableSwitch.UUID)
          setTimeout(function () {
            cx.emit("change", {
              oldValue: cx.value,
              newValue: (cx.value = cx.zway_setValueOnVDev(
                cx.props["minValue"]
              )),
            });
          }, accessory.platform.blinkTime);
  ...
  configureService: function (service, vdev) {
    ...
    for (var i = 0; i < service.optionalCharacteristics.length; i++) {
      ...
      var othercx = null,
        othercxs = this.platform.cxVDevMap[vdev.id];
      if (othercxs)
        for (var j = 0; j < othercxs.length; j++)
          if (othercxs[j].UUID === cx.UUID) othercx = othercxs[j];
      if (othercx) continue;
  ...
  getServices: function () {
    ...
    if (services.length === 1) {
      debug(
        "WARN: Only the InformationService was successfully configured for " +
          vdevPrimary.id +
          "! No device services available!"
      );
      return services;
    }
    ...
    if (
      this.platform.getTagValue(vdevPrimary, "Interlock") &&
      services.length > 1
    ) {
      var ilsvc = new Service.Switch(
        "Interlock",
        vdevPrimary.id + "_interlock"
      );
      ...
      ilcx.on(
        ...
        function (ev) {
          debug(
            "Interlock for device " +
              vdevPrimary.metrics.title +
              " changed from " +
              ev.oldValue +
              " to " +
              ev.newValue +
    ...
    if (this.devDesc.extras["switchMultilevel"])
      for (var i = 0; i < this.devDesc.extras["switchMultilevel"].length; i++) {
        var xvdev =
          this.devDesc.devices[this.devDesc.extras["switchMultilevel"][i]];
    ...
    if (this.platform.splitServices) {
      if (this.devDesc.types["battery.Battery"]) {
        services = services.concat(
          this.getVDevServices(
            this.devDesc.devices[this.devDesc.types["battery.Battery"]]
          )
        );
      }
      ...
      var tempSensor =
        this.devDesc.types["sensorMultilevel.Temperature"] !== undefined
          ? this.devDesc.devices[
              this.devDesc.types["sensorMultilevel.Temperature"]
            ]
          : false;
      ...
      var rhSensor =
        this.devDesc.types["sensorMultilevel.Humidity"] !== undefined
          ? this.devDesc.devices[
              this.devDesc.types["sensorMultilevel.Humidity"]
            ]
          : false;
      ...
      var lightSensor =
        this.devDesc.types["sensorMultilevel.Luminiscence"] !== undefined
          ? this.devDesc.devices[
              this.devDesc.types["sensorMultilevel.Luminiscence"]
            ]
          : false;
      ...
      var wattSensor =
        this.devDesc.types["sensorMultilevel.meterElectric_watt"] !== undefined
          ? this.devDesc.devices[
              this.devDesc.types["sensorMultilevel.meterElectric_watt"]
            ]
          : false;
      ...
      var existingCxUUIDs = {};
      for (var i = 0; i < service.characteristics.length; i++)
        existingCxUUIDs[service.characteristics[i].UUID] = true;
      ...
      for (var i = 0; i < this.devDesc.devices.length; i++) {
        ...
        var extraCxClasses =
          this.extraCharacteristicsMap[ZWayServerPlatform.getVDevTypeKey(vdev)];
        var extraCxs = [];
